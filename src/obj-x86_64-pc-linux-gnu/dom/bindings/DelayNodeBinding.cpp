...
bool
DelayOptions::Init(BindingCallContext& cx, JS::Handle<JS::Value> val, const char* sourceDescription, bool passedToJSImpl)
{
  ...
  if (!isNull && !temp->isUndefined()) {
    if (!ValueToPrimitive<double, eDefault>(cx, temp.ref(), "'delayTime' member of DelayOptions", &mDelayTime)) {
      return false;
    } else if (!mozilla::IsFinite(mDelayTime)) {
      cx.ThrowErrorMessage<MSG_NOT_FINITE>("'delayTime' member of DelayOptions");
      return false;
    }
  } else {
    mDelayTime = 0.0;
  ...
  if (!isNull && !temp->isUndefined()) {
    if (!ValueToPrimitive<double, eDefault>(cx, temp.ref(), "'maxDelayTime' member of DelayOptions", &mMaxDelayTime)) {
      return false;
    } else if (!mozilla::IsFinite(mMaxDelayTime)) {
      cx.ThrowErrorMessage<MSG_NOT_FINITE>("'maxDelayTime' member of DelayOptions");
      return false;
    }
  } else {
    mMaxDelayTime = 1.0;
...
bool
DelayOptions::Init(JSContext* cx_, JS::Handle<JS::Value> val, const char* sourceDescription, bool passedToJSImpl)
{
...
bool
DelayOptions::operator==(const DelayOptions& aOther) const
{
  if (mDelayTime != aOther.mDelayTime) {
      return false;
  }
  if (mMaxDelayTime != aOther.mMaxDelayTime) {
      return false;
  }
  return true;
}
...
get_delayTime(JSContext* cx, JS::Handle<JSObject*> obj, void* void_self, JSJitGetterCallArgs args)
{
  AUTO_PROFILER_LABEL_DYNAMIC_FAST(
    "DelayNode", "delayTime", DOM, cx,
    uint32_t(js::ProfilingStackFrame::Flags::STRING_TEMPLATE_GETTER) |
    uint32_t(js::ProfilingStackFrame::Flags::RELEVANT_FOR_JS));

  auto* self = static_cast<mozilla::dom::DelayNode*>(void_self);
  auto result(StrongOrRawPtr<mozilla::dom::AudioParam>(MOZ_KnownLive(self)->DelayTime()));
...
static const JSPropertySpec sAttributes_specs[] = {
  JSPropertySpec::nativeAccessors("delayTime", JSPROP_ENUMERATE, GenericGetter<NormalThisPolicy, ThrowExceptions>, &delayTime_getterinfo, nullptr, nullptr),
  JS_STRING_SYM_PS(toStringTag, "DelayNode", JSPROP_READONLY),
  JS_PS_END
};
...
{
  BindingCallContext cx(cx_, "DelayNode constructor");
  AUTO_PROFILER_LABEL_DYNAMIC_FAST(
    "DelayNode", "constructor", DOM, cx,
    uint32_t(js::ProfilingStackFrame::Flags::RELEVANT_FOR_JS));
  ...
  if (!args.requireAtLeast(cx, "DelayNode constructor", 1)) {
    return false;
  }
  ...
  if (MOZ_UNLIKELY(rv.MaybeSetPendingException(cx, "DelayNode constructor"))) {
    return false;
  }
  MOZ_ASSERT(!JS_IsExceptionPending(cx));
...
static const DOMJSClass sClass = {
  ...
  std::is_base_of_v<nsISupports, mozilla::dom::DelayNode>,
  sNativePropertyHooks,
...
{
  static_assert(!std::is_base_of_v<NonRefcountedDOMObject, mozilla::dom::DelayNode>,
