...
bool
StereoPannerOptions::Init(BindingCallContext& cx, JS::Handle<JS::Value> val, const char* sourceDescription, bool passedToJSImpl)
{
  ...
  if (!isNull && !temp->isUndefined()) {
    if (!ValueToPrimitive<float, eDefault>(cx, temp.ref(), "'pan' member of StereoPannerOptions", &mPan)) {
      return false;
    } else if (!mozilla::IsFinite(mPan)) {
      cx.ThrowErrorMessage<MSG_NOT_FINITE>("'pan' member of StereoPannerOptions");
      return false;
    }
  } else {
    mPan = 0.0F;
...
bool
StereoPannerOptions::Init(JSContext* cx_, JS::Handle<JS::Value> val, const char* sourceDescription, bool passedToJSImpl)
{
...
bool
StereoPannerOptions::operator==(const StereoPannerOptions& aOther) const
{
  if (mPan != aOther.mPan) {
      return false;
  }
...
get_pan(JSContext* cx, JS::Handle<JSObject*> obj, void* void_self, JSJitGetterCallArgs args)
{
  AUTO_PROFILER_LABEL_DYNAMIC_FAST(
    "StereoPannerNode", "pan", DOM, cx,
    uint32_t(js::ProfilingStackFrame::Flags::STRING_TEMPLATE_GETTER) |
    uint32_t(js::ProfilingStackFrame::Flags::RELEVANT_FOR_JS));

  auto* self = static_cast<mozilla::dom::StereoPannerNode*>(void_self);
  auto result(StrongOrRawPtr<mozilla::dom::AudioParam>(MOZ_KnownLive(self)->Pan()));
...
{
  AUTO_PROFILER_LABEL_DYNAMIC_FAST(
    "StereoPannerNode", "passThrough", DOM, cx,
    uint32_t(js::ProfilingStackFrame::Flags::STRING_TEMPLATE_GETTER) |
    uint32_t(js::ProfilingStackFrame::Flags::RELEVANT_FOR_JS));
...
static const JSPropertySpec sAttributes_specs[] = {
  JSPropertySpec::nativeAccessors("pan", JSPROP_ENUMERATE, GenericGetter<NormalThisPolicy, ThrowExceptions>, &pan_getterinfo, nullptr, nullptr),
  JS_STRING_SYM_PS(toStringTag, "StereoPannerNode", JSPROP_READONLY),
  JS_PS_END
};
...
{
  BindingCallContext cx(cx_, "StereoPannerNode constructor");
  AUTO_PROFILER_LABEL_DYNAMIC_FAST(
    "StereoPannerNode", "constructor", DOM, cx,
    uint32_t(js::ProfilingStackFrame::Flags::RELEVANT_FOR_JS));
  ...
  if (!args.requireAtLeast(cx, "StereoPannerNode constructor", 1)) {
    return false;
  }
  ...
  if (MOZ_UNLIKELY(rv.MaybeSetPendingException(cx, "StereoPannerNode constructor"))) {
    return false;
  }
  MOZ_ASSERT(!JS_IsExceptionPending(cx));
...
static const DOMJSClass sClass = {
  ...
  std::is_base_of_v<nsISupports, mozilla::dom::StereoPannerNode>,
  sNativePropertyHooks,
...
{
  static_assert(!std::is_base_of_v<NonRefcountedDOMObject, mozilla::dom::StereoPannerNode>,
