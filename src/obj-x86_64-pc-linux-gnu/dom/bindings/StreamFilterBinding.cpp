...
{
  MOZ_ASSERT(uint32_t(aArgument) < ArrayLength(StreamFilterStatusValues::strings));
  JSString* resultStr =
    JS_NewStringCopyN(aCx, StreamFilterStatusValues::strings[uint32_t(aArgument)].value,
                      StreamFilterStatusValues::strings[uint32_t(aArgument)].length);
  if (!resultStr) {
    return false;
  }
...
write(JSContext* cx_, JS::Handle<JSObject*> obj, void* void_self, const JSJitMethodCallArgs& args)
{
  ...
  ArrayBufferOrUint8ArrayArgument arg0_holder(arg0);
  {
    bool done = false, failed = false, tryNext;
    if (args[0].isObject()) {
      done = (failed = !arg0_holder.TrySetToArrayBuffer(cx, args[0], tryNext, false)) || !tryNext ||
             (failed = !arg0_holder.TrySetToUint8Array(cx, args[0], tryNext, false)) || !tryNext;
    ...
    if (!done) {
      cx.ThrowErrorMessage<MSG_NOT_IN_UNION>("Argument 1", "ArrayBuffer, Uint8Array");
      return false;
    }
  }
  FastErrorResult rv;
...
static const JSPropertySpec sAttributes_specs[] = {
  ...
  JS_STRING_SYM_PS(toStringTag, "StreamFilter", JSPROP_READONLY),
  JS_PS_END
};
...
Wrap(JSContext* aCx, mozilla::extensions::StreamFilter* aObject, nsWrapperCache* aCache, JS::Handle<JSObject*> aGivenProto, JS::MutableHandle<JSObject*> aReflector)
{
  ...
  MOZ_ASSERT(static_cast<mozilla::extensions::StreamFilter*>(aObject) ==
             reinterpret_cast<mozilla::extensions::StreamFilter*>(aObject),
             "Multiple inheritance for mozilla::extensions::StreamFilter is broken.");
