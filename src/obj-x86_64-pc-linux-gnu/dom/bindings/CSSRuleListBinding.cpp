...
{
  AUTO_PROFILER_LABEL_DYNAMIC_FAST(
    "CSSRuleList", "item", DOM, cx,
    uint32_t(js::ProfilingStackFrame::Flags::STRING_TEMPLATE_METHOD) |
    uint32_t(js::ProfilingStackFrame::Flags::RELEVANT_FOR_JS));

  auto* self = static_cast<mozilla::dom::CSSRuleList*>(void_self);
  if (!args.requireAtLeast(cx, "CSSRuleList.item", 1)) {
    return false;
  }
  uint32_t arg0;
  ...
  auto result(StrongOrRawPtr<mozilla::css::Rule>(MOZ_KnownLive(self)->Item(arg0)));
  MOZ_ASSERT(!JS_IsExceptionPending(cx));
  if (!result) {
    args.rval().setNull();
    return true;
...
static const JSPropertySpec sAttributes_specs[] = {
  ...
  JS_STRING_SYM_PS(toStringTag, "CSSRuleList", JSPROP_READONLY),
  JS_PS_END
};
...
static_assert(std::is_base_of_v<nsISupports, mozilla::dom::CSSRuleList>,
...
UnwrapProxy(JSObject* obj)
{
  ...
  return static_cast<mozilla::dom::CSSRuleList*>(js::GetProxyReservedSlot(obj, DOM_OBJECT_SLOT).toPrivate());
}
...
DOMProxyHandler::getOwnPropDescriptor(JSContext* cx, JS::Handle<JSObject*> proxy, JS::Handle<jsid> id, bool ignoreNamedProps, JS::MutableHandle<JS::PropertyDescriptor> desc) const
{
  ...
  if (IsArrayIndex(index)) {
    ...
    bool found = false;
    auto result(StrongOrRawPtr<mozilla::css::Rule>(MOZ_KnownLive(self)->IndexedGetter(index, found)));
    MOZ_ASSERT(!JS_IsExceptionPending(cx));
...
static const DOMJSClass sClass = {
  ...
  std::is_base_of_v<nsISupports, mozilla::dom::CSSRuleList>,
  sNativePropertyHooks,
...
{
  static_assert(!std::is_base_of_v<NonRefcountedDOMObject, mozilla::dom::CSSRuleList>,
