...
bool
WorkerOptions::Init(BindingCallContext& cx, JS::Handle<JS::Value> val, const char* sourceDescription, bool passedToJSImpl)
{
...
bool
WorkerOptions::Init(JSContext* cx_, JS::Handle<JS::Value> val, const char* sourceDescription, bool passedToJSImpl)
{
...
bool
WorkerOptions::operator==(const WorkerOptions& aOther) const
{
...
static const JSPropertySpec sAttributes_specs[] = {
  JS_STRING_SYM_PS(toStringTag, "ChromeWorker", JSPROP_READONLY),
  JS_PS_END
};
...
_constructor(JSContext* cx, unsigned argc, JS::Value* vp)
{
  AUTO_PROFILER_LABEL_DYNAMIC_FAST(
    "ChromeWorker", "constructor", DOM, cx,
  ...
  if (!args.requireAtLeast(cx, "ChromeWorker constructor", 1)) {
    return false;
  }
  ...
  if (MOZ_UNLIKELY(rv.MaybeSetPendingException(cx, "ChromeWorker constructor"))) {
    return false;
  }
...
static const DOMJSClass sClass = {
  ...
  std::is_base_of_v<nsISupports, mozilla::dom::ChromeWorker>,
  sNativePropertyHooks,
...
{
  static_assert(!std::is_base_of_v<NonRefcountedDOMObject, mozilla::dom::ChromeWorker>,
...
{
  AUTO_PROFILER_LABEL_DYNAMIC_FAST(
    "Worker", "terminate", DOM, cx,
    uint32_t(js::ProfilingStackFrame::Flags::STRING_TEMPLATE_METHOD) |
    uint32_t(js::ProfilingStackFrame::Flags::RELEVANT_FOR_JS));

  auto* self = static_cast<mozilla::dom::Worker*>(void_self);
  // NOTE: This assert does NOT call the function.
  static_assert(std::is_void_v<decltype(MOZ_KnownLive(self)->Terminate())>, "Should be returning void here");
  MOZ_KnownLive(self)->Terminate();
  MOZ_ASSERT(!JS_IsExceptionPending(cx));
...
{
  BindingCallContext cx(cx_, "Worker.postMessage");
  AUTO_PROFILER_LABEL_DYNAMIC_FAST(
    "Worker", "postMessage", DOM, cx,
    uint32_t(js::ProfilingStackFrame::Flags::STRING_TEMPLATE_METHOD) |
    uint32_t(js::ProfilingStackFrame::Flags::RELEVANT_FOR_JS));
  ...
  switch (argcount) {
    case 1: {
      ...
      if (MOZ_UNLIKELY(rv.MaybeSetPendingException(cx, "Worker.postMessage"))) {
        return false;
      }
      MOZ_ASSERT(!JS_IsExceptionPending(cx));
      args.rval().setUndefined();
...
static const JSPropertySpec sAttributes_specs[] = {
  ...
  JS_STRING_SYM_PS(toStringTag, "Worker", JSPROP_READONLY),
  JS_PS_END
};
...
{
  BindingCallContext cx(cx_, "Worker constructor");
  AUTO_PROFILER_LABEL_DYNAMIC_FAST(
    "Worker", "constructor", DOM, cx,
    uint32_t(js::ProfilingStackFrame::Flags::RELEVANT_FOR_JS));
  ...
  if (!args.requireAtLeast(cx, "Worker constructor", 1)) {
    return false;
  }
  GlobalObject global(cx, obj);
  if (global.Failed()) {
  ...
  if (MOZ_UNLIKELY(rv.MaybeSetPendingException(cx, "Worker constructor"))) {
    return false;
  }
  MOZ_ASSERT(!JS_IsExceptionPending(cx));
  static_assert(!std::is_pointer_v<decltype(result)>,
...
static const DOMJSClass sClass = {
  ...
  std::is_base_of_v<nsISupports, mozilla::dom::Worker>,
  sNativePropertyHooks,
...
{
  static_assert(!std::is_base_of_v<NonRefcountedDOMObject, mozilla::dom::Worker>,
                "Shouldn't have wrappercached things that are not refcounted.");
  MOZ_ASSERT(static_cast<mozilla::dom::Worker*>(aObject) ==
             reinterpret_cast<mozilla::dom::Worker*>(aObject),
             "Multiple inheritance for mozilla::dom::Worker is broken.");
