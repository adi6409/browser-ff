...
get_description(JSContext* cx, JS::Handle<JSObject*> obj, void* void_self, JSJitGetterCallArgs args)
{
  AUTO_PROFILER_LABEL_DYNAMIC_FAST(
    "Plugin", "description", DOM, cx,
    uint32_t(js::ProfilingStackFrame::Flags::STRING_TEMPLATE_GETTER) |
    uint32_t(js::ProfilingStackFrame::Flags::RELEVANT_FOR_JS));

  auto* self = static_cast<nsPluginElement*>(void_self);
  DOMString result;
...
item(JSContext* cx, JS::Handle<JSObject*> obj, void* void_self, const JSJitMethodCallArgs& args)
{
  ...
  auto* self = static_cast<nsPluginElement*>(void_self);
  if (!args.requireAtLeast(cx, "Plugin.item", 1)) {
    return false;
  }
  uint32_t arg0;
  ...
  auto result(StrongOrRawPtr<nsMimeType>(MOZ_KnownLive(self)->Item(arg0)));
  MOZ_ASSERT(!JS_IsExceptionPending(cx));
  if (!result) {
    args.rval().setNull();
    return true;
...
{
  AUTO_PROFILER_LABEL_DYNAMIC_FAST(
    "Plugin", "namedItem", DOM, cx,
    uint32_t(js::ProfilingStackFrame::Flags::STRING_TEMPLATE_METHOD) |
    uint32_t(js::ProfilingStackFrame::Flags::RELEVANT_FOR_JS));

  auto* self = static_cast<nsPluginElement*>(void_self);
  if (!args.requireAtLeast(cx, "Plugin.namedItem", 1)) {
    return false;
  }
  ...
  auto result(StrongOrRawPtr<nsMimeType>(MOZ_KnownLive(self)->NamedItem(NonNullHelper(Constify(arg0)))));
  MOZ_ASSERT(!JS_IsExceptionPending(cx));
  if (!result) {
    args.rval().setNull();
    return true;
...
static const JSPropertySpec sAttributes_specs[] = {
  ...
  JS_STRING_SYM_PS(toStringTag, "Plugin", JSPROP_READONLY),
  JS_PS_END
};
...
static_assert(std::is_base_of_v<nsISupports, nsPluginElement>,
...
DOMProxyHandler::getOwnPropDescriptor(JSContext* cx, JS::Handle<JSObject*> proxy, JS::Handle<jsid> id, bool ignoreNamedProps, JS::MutableHandle<JS::PropertyDescriptor> desc) const
{
  ...
  if (IsArrayIndex(index)) {
    ...
    bool found = false;
    auto result(StrongOrRawPtr<nsMimeType>(MOZ_KnownLive(self)->IndexedGetter(index, found)));
    MOZ_ASSERT(!JS_IsExceptionPending(cx));
  ...
  if (callNamedGetter) {
    ...
    if (!isSymbol) {
      nsPluginElement* self = UnwrapProxy(proxy);
      bool found = false;
      auto result(StrongOrRawPtr<nsMimeType>(MOZ_KnownLive(self)->NamedGetter(NonNullHelper(Constify(name)), found)));
      MOZ_ASSERT(!JS_IsExceptionPending(cx));
...
static const DOMJSClass sClass = {
  ...
  std::is_base_of_v<nsISupports, nsPluginElement>,
  sNativePropertyHooks,
...
{
  static_assert(!std::is_base_of_v<NonRefcountedDOMObject, nsPluginElement>,
