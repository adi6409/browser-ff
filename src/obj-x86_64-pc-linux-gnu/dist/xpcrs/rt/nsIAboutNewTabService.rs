//
// DO NOT EDIT.  THIS FILE IS GENERATED FROM $SRCDIR/browser/components/newtab/nsIAboutNewTabService.idl
//


/// `interface nsIAboutNewTabService : nsISupports`
///


// The actual type definition for the interface. This struct has methods
// declared on it which will call through its vtable. You never want to pass
// this type around by value, always pass it behind a reference.

#[repr(C)]
pub struct nsIAboutNewTabService {
    vtable: *const nsIAboutNewTabServiceVTable,

    /// This field is a phantomdata to ensure that the VTable type and any
    /// struct containing it is not safe to send across threads, as XPCOM is
    /// generally not threadsafe.
    ///
    /// XPCOM interfaces in general are not safe to send across threads.
    __nosync: ::std::marker::PhantomData<::std::rc::Rc<u8>>,
}

// Implementing XpCom for an interface exposes its IID, which allows for easy
// use of the `.query_interface<T>` helper method. This also defines that
// method for nsIAboutNewTabService.
unsafe impl XpCom for nsIAboutNewTabService {
    const IID: nsIID = nsID(0xdfcd2adc, 0x7867, 0x4d3a,
        [0xba, 0x70, 0x17, 0x50, 0x1f, 0x20, 0x81, 0x42]);
}

// We need to implement the RefCounted trait so we can be used with `RefPtr`.
// This trait teaches `RefPtr` how to manage our memory.
unsafe impl RefCounted for nsIAboutNewTabService {
    #[inline]
    unsafe fn addref(&self) {
        self.AddRef();
    }
    #[inline]
    unsafe fn release(&self) {
        self.Release();
    }
}

// This trait is implemented on all types which can be coerced to from nsIAboutNewTabService.
// It is used in the implementation of `fn coerce<T>`. We hide it from the
// documentation, because it clutters it up a lot.
#[doc(hidden)]
pub trait nsIAboutNewTabServiceCoerce {
    /// Cheaply cast a value of this type from a `nsIAboutNewTabService`.
    fn coerce_from(v: &nsIAboutNewTabService) -> &Self;
}

// The trivial implementation: We can obviously coerce ourselves to ourselves.
impl nsIAboutNewTabServiceCoerce for nsIAboutNewTabService {
    #[inline]
    fn coerce_from(v: &nsIAboutNewTabService) -> &Self {
        v
    }
}

impl nsIAboutNewTabService {
    /// Cast this `nsIAboutNewTabService` to one of its base interfaces.
    #[inline]
    pub fn coerce<T: nsIAboutNewTabServiceCoerce>(&self) -> &T {
        T::coerce_from(self)
    }
}

// Every interface struct type implements `Deref` to its base interface. This
// causes methods on the base interfaces to be directly avaliable on the
// object. For example, you can call `.AddRef` or `.QueryInterface` directly
// on any interface which inherits from `nsISupports`.
impl ::std::ops::Deref for nsIAboutNewTabService {
    type Target = nsISupports;
    #[inline]
    fn deref(&self) -> &nsISupports {
        unsafe {
            ::std::mem::transmute(self)
        }
    }
}

// Ensure we can use .coerce() to cast to our base types as well. Any type which
// our base interface can coerce from should be coercable from us as well.
impl<T: nsISupportsCoerce> nsIAboutNewTabServiceCoerce for T {
    #[inline]
    fn coerce_from(v: &nsIAboutNewTabService) -> &Self {
        T::coerce_from(v)
    }
}

// This struct represents the interface's VTable. A pointer to a statically
// allocated version of this struct is at the beginning of every nsIAboutNewTabService
// object. It contains one pointer field for each method in the interface. In
// the case where we can't generate a binding for a method, we include a void
// pointer.
#[doc(hidden)]
#[repr(C)]
pub struct nsIAboutNewTabServiceVTable {
    /// We need to include the members from the base interface's vtable at the start
    /// of the VTable definition.
    pub __base: nsISupportsVTable,

    /* readonly attribute ACString defaultURL; */
    pub GetDefaultURL: unsafe extern "system" fn (this: *const nsIAboutNewTabService, aDefaultURL: *mut ::nsstring::nsACString) -> ::nserror::nsresult,

    /* nsIChannel aboutHomeChannel (in nsIURI aURI, in nsILoadInfo aLoadInfo); */
    pub AboutHomeChannel: unsafe extern "system" fn (this: *const nsIAboutNewTabService, aURI: *const nsIURI, aLoadInfo: *const nsILoadInfo, _retval: *mut*const nsIChannel) -> ::nserror::nsresult,

    /* readonly attribute ACString welcomeURL; */
    pub GetWelcomeURL: unsafe extern "system" fn (this: *const nsIAboutNewTabService, aWelcomeURL: *mut ::nsstring::nsACString) -> ::nserror::nsresult,
}


// The implementations of the function wrappers which are exposed to rust code.
// Call these methods rather than manually calling through the VTable struct.
impl nsIAboutNewTabService {

    /// ```text
    /// /**
    ///    * Returns the default URL (local or activity stream depending on pref)
    ///    */
    /// ```
    ///

    /// `readonly attribute ACString defaultURL;`
    #[inline]
    pub unsafe fn GetDefaultURL(&self, aDefaultURL: *mut ::nsstring::nsACString) -> ::nserror::nsresult {
        ((*self.vtable).GetDefaultURL)(self, aDefaultURL)
    }


    /// ```text
    /// /**
    ///    * In the "privileged about content process", if about:home is being
    ///    * retrieved, the AboutRedirector will call this function to get the
    ///    * nsIChannel for the document. This gives the nsIAboutNewTabService
    ///    * the opportunity to provide a cached document for about:home. If
    ///    * no cache exists, the nsIChannel will be for the normal dynamically
    ///    * generated about:home document.
    ///    */
    /// ```
    ///

    /// `nsIChannel aboutHomeChannel (in nsIURI aURI, in nsILoadInfo aLoadInfo);`
    #[inline]
    pub unsafe fn AboutHomeChannel(&self, aURI: *const nsIURI, aLoadInfo: *const nsILoadInfo, _retval: *mut*const nsIChannel) -> ::nserror::nsresult {
        ((*self.vtable).AboutHomeChannel)(self, aURI, aLoadInfo, _retval)
    }


    /// ```text
    /// /**
    ///    * Returns the about:welcome URL.
    ///    */
    /// ```
    ///

    /// `readonly attribute ACString welcomeURL;`
    #[inline]
    pub unsafe fn GetWelcomeURL(&self, aWelcomeURL: *mut ::nsstring::nsACString) -> ::nserror::nsresult {
        ((*self.vtable).GetWelcomeURL)(self, aWelcomeURL)
    }


}


