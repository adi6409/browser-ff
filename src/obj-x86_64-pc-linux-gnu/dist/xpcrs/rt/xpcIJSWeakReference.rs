...
// method for xpcIJSWeakReference.
unsafe impl XpCom for xpcIJSWeakReference {
    const IID: nsIID = nsID(0x75767928, 0xecb1, 0x4e6c,
...
// This trait is implemented on all types which can be coerced to from xpcIJSWeakReference.
// It is used in the implementation of `fn coerce<T>`. We hide it from the
// documentation, because it clutters it up a lot.
#[doc(hidden)]
pub trait xpcIJSWeakReferenceCoerce {
    /// Cheaply cast a value of this type from a `xpcIJSWeakReference`.
...
impl xpcIJSWeakReference {
    /// Cast this `xpcIJSWeakReference` to one of its base interfaces.
    #[inline]
...
// allocated version of this struct is at the beginning of every xpcIJSWeakReference
