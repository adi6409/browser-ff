//
// DO NOT EDIT.  THIS FILE IS GENERATED FROM $SRCDIR/netwerk/streamconv/nsITXTToHTMLConv.idl
//


/// `interface nsITXTToHTMLConv : nsIStreamConverter`
///


// The actual type definition for the interface. This struct has methods
// declared on it which will call through its vtable. You never want to pass
// this type around by value, always pass it behind a reference.

#[repr(C)]
pub struct nsITXTToHTMLConv {
    vtable: *const nsITXTToHTMLConvVTable,

    /// This field is a phantomdata to ensure that the VTable type and any
    /// struct containing it is not safe to send across threads, as XPCOM is
    /// generally not threadsafe.
    ///
    /// XPCOM interfaces in general are not safe to send across threads.
    __nosync: ::std::marker::PhantomData<::std::rc::Rc<u8>>,
}

// Implementing XpCom for an interface exposes its IID, which allows for easy
// use of the `.query_interface<T>` helper method. This also defines that
// method for nsITXTToHTMLConv.
unsafe impl XpCom for nsITXTToHTMLConv {
    const IID: nsIID = nsID(0x933355f6, 0x1dd2, 0x11b2,
        [0xa9, 0xb0, 0xd3, 0x35, 0xb9, 0xe3, 0x59, 0x83]);
}

// We need to implement the RefCounted trait so we can be used with `RefPtr`.
// This trait teaches `RefPtr` how to manage our memory.
unsafe impl RefCounted for nsITXTToHTMLConv {
    #[inline]
    unsafe fn addref(&self) {
        self.AddRef();
    }
    #[inline]
    unsafe fn release(&self) {
        self.Release();
    }
}

// This trait is implemented on all types which can be coerced to from nsITXTToHTMLConv.
// It is used in the implementation of `fn coerce<T>`. We hide it from the
// documentation, because it clutters it up a lot.
#[doc(hidden)]
pub trait nsITXTToHTMLConvCoerce {
    /// Cheaply cast a value of this type from a `nsITXTToHTMLConv`.
    fn coerce_from(v: &nsITXTToHTMLConv) -> &Self;
}

// The trivial implementation: We can obviously coerce ourselves to ourselves.
impl nsITXTToHTMLConvCoerce for nsITXTToHTMLConv {
    #[inline]
    fn coerce_from(v: &nsITXTToHTMLConv) -> &Self {
        v
    }
}

impl nsITXTToHTMLConv {
    /// Cast this `nsITXTToHTMLConv` to one of its base interfaces.
    #[inline]
    pub fn coerce<T: nsITXTToHTMLConvCoerce>(&self) -> &T {
        T::coerce_from(self)
    }
}

// Every interface struct type implements `Deref` to its base interface. This
// causes methods on the base interfaces to be directly avaliable on the
// object. For example, you can call `.AddRef` or `.QueryInterface` directly
// on any interface which inherits from `nsISupports`.
impl ::std::ops::Deref for nsITXTToHTMLConv {
    type Target = nsIStreamConverter;
    #[inline]
    fn deref(&self) -> &nsIStreamConverter {
        unsafe {
            ::std::mem::transmute(self)
        }
    }
}

// Ensure we can use .coerce() to cast to our base types as well. Any type which
// our base interface can coerce from should be coercable from us as well.
impl<T: nsIStreamConverterCoerce> nsITXTToHTMLConvCoerce for T {
    #[inline]
    fn coerce_from(v: &nsITXTToHTMLConv) -> &Self {
        T::coerce_from(v)
    }
}

// This struct represents the interface's VTable. A pointer to a statically
// allocated version of this struct is at the beginning of every nsITXTToHTMLConv
// object. It contains one pointer field for each method in the interface. In
// the case where we can't generate a binding for a method, we include a void
// pointer.
#[doc(hidden)]
#[repr(C)]
pub struct nsITXTToHTMLConvVTable {
    /// We need to include the members from the base interface's vtable at the start
    /// of the VTable definition.
    pub __base: nsIStreamConverterVTable,

    /* void setTitle (in wstring text); */
    pub SetTitle: unsafe extern "system" fn (this: *const nsITXTToHTMLConv, text: *const i16) -> ::nserror::nsresult,

    /* void preFormatHTML (in boolean value); */
    pub PreFormatHTML: unsafe extern "system" fn (this: *const nsITXTToHTMLConv, value: bool) -> ::nserror::nsresult,
}


// The implementations of the function wrappers which are exposed to rust code.
// Call these methods rather than manually calling through the VTable struct.
impl nsITXTToHTMLConv {

    /// ```text
    /// /**
    ///    * @param text: Title to set for the HTML document.  Only applicable if
    ///    *              preFormatHTML(true) is called.
    ///    * @result      The given title will be used to form an HTML document
    ///    *              from the plain text document.
    ///    */
    /// ```
    ///

    /// `void setTitle (in wstring text);`
    #[inline]
    pub unsafe fn SetTitle(&self, text: *const i16) -> ::nserror::nsresult {
        ((*self.vtable).SetTitle)(self, text)
    }


    /// ```text
    /// /**
    ///    * @param value: true to use an HTML header and footer on the document,
    ///    *               false to omit it.
    ///    * @result       The document will use a header and footer if value is
    ///    *               true.
    ///    */
    /// ```
    ///

    /// `void preFormatHTML (in boolean value);`
    #[inline]
    pub unsafe fn PreFormatHTML(&self, value: bool) -> ::nserror::nsresult {
        ((*self.vtable).PreFormatHTML)(self, value)
    }


}


