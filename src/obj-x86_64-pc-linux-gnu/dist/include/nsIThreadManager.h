...
class nsIThreadManager : public nsISupports {
 public:
  ...
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD NewThread(uint32_t creationFlags, uint32_t stackSize, nsIThread **_retval) = 0;
  ...
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD GetMainThread(nsIThread **aMainThread) = 0;

  /* readonly attribute nsIThread currentThread; */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD GetCurrentThread(nsIThread **aCurrentThread) = 0;

  /* [optional_argc] void dispatchToMainThread (in nsIRunnable event, [optional] in uint32_t priority); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD DispatchToMainThread(nsIRunnable *event, uint32_t priority, uint8_t _argc) = 0;

  /* void idleDispatchToMainThread (in nsIRunnable event, [optional] in uint32_t timeout); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD IdleDispatchToMainThread(nsIRunnable *event, uint32_t timeout) = 0;

  /* void spinEventLoopUntil (in nsINestedEventLoopCondition condition); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD SpinEventLoopUntil(nsINestedEventLoopCondition *condition) = 0;

  /* void spinEventLoopUntilOrShutdown (in nsINestedEventLoopCondition condition); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD SpinEventLoopUntilOrShutdown(nsINestedEventLoopCondition *condition) = 0;

  /* void spinEventLoopUntilEmpty (); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD SpinEventLoopUntilEmpty(void) = 0;

  /* readonly attribute nsIEventTarget mainThreadEventTarget; */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD GetMainThreadEventTarget(nsIEventTarget **aMainThreadEventTarget) = 0;
...
#define NS_DECL_NSITHREADMANAGER \
  ...
  NS_IMETHOD SpinEventLoopUntilOrShutdown(nsINestedEventLoopCondition *condition) override; \
...
#define NS_DECL_NON_VIRTUAL_NSITHREADMANAGER \
  ...
  nsresult SpinEventLoopUntilOrShutdown(nsINestedEventLoopCondition *condition); \
...
#define NS_FORWARD_NSITHREADMANAGER(_to) \
  ...
  NS_IMETHOD SpinEventLoopUntilOrShutdown(nsINestedEventLoopCondition *condition) override { return _to SpinEventLoopUntilOrShutdown(condition); } \
...
#define NS_FORWARD_SAFE_NSITHREADMANAGER(_to) \
  ...
  NS_IMETHOD SpinEventLoopUntilOrShutdown(nsINestedEventLoopCondition *condition) override { return !_to ? NS_ERROR_NULL_POINTER : _to->SpinEventLoopUntilOrShutdown(condition); } \
