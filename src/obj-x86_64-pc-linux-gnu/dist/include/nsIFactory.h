...
class NS_NO_VTABLE nsIFactory : public nsISupports {
 public:
  ...
  using ScriptableInterfaceType = nsIFactory;

  /* void createInstance (in nsISupports aOuter, in nsIIDRef iid, [iid_is (iid), retval] out nsQIResult result); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD CreateInstance(nsISupports *aOuter, const nsIID & iid, void * * result) = 0;

  /* void lockFactory (in boolean lock); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD LockFactory(bool lock) = 0;
