...
class NS_NO_VTABLE nsISerializable : public nsISupports {
 public:
  ...
  using ScriptableInterfaceType = nsISerializable;

  /* [must_use] void read (in nsIObjectInputStream aInputStream); */
  JS_HAZ_CAN_RUN_SCRIPT [[nodiscard]] NS_IMETHOD Read(nsIObjectInputStream *aInputStream) = 0;

  /* void write (in nsIObjectOutputStream aOutputStream); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD Write(nsIObjectOutputStream *aOutputStream) = 0;
...
#define NS_DECL_NSISERIALIZABLE \
  [[nodiscard]] NS_IMETHOD Read(nsIObjectInputStream *aInputStream) override; \
...
#define NS_DECL_NON_VIRTUAL_NSISERIALIZABLE \
  [[nodiscard]] nsresult Read(nsIObjectInputStream *aInputStream); \
  nsresult Write(nsIObjectOutputStream *aOutputStream); 

/* Use this macro to declare functions that forward the behavior of this interface to another object. */
#define NS_FORWARD_NSISERIALIZABLE(_to) \
  [[nodiscard]] NS_IMETHOD Read(nsIObjectInputStream *aInputStream) override { return _to Read(aInputStream); } \
...
#define NS_FORWARD_SAFE_NSISERIALIZABLE(_to) \
  [[nodiscard]] NS_IMETHOD Read(nsIObjectInputStream *aInputStream) override { return !_to ? NS_ERROR_NULL_POINTER : _to->Read(aInputStream); } \
