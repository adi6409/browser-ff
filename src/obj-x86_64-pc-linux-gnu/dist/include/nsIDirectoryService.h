...
class NS_NO_VTABLE nsIDirectoryServiceProvider : public nsISupports {
 public:
  ...
  using ScriptableInterfaceType = nsIDirectoryServiceProvider;

  /* nsIFile getFile (in string prop, out boolean persistent); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD GetFile(const char * prop, bool *persistent, nsIFile **_retval) = 0;
...
class NS_NO_VTABLE nsIDirectoryServiceProvider2 : public nsIDirectoryServiceProvider {
 public:
  ...
  /* Used by ToJSValue to check which scriptable interface is implemented. */
  using ScriptableInterfaceType = nsIDirectoryServiceProvider2;

  /* nsISimpleEnumerator getFiles (in string prop); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD GetFiles(const char * prop, nsISimpleEnumerator **_retval) = 0;
...
class NS_NO_VTABLE nsIDirectoryService : public nsISupports {
 public:
  ...
  /* Used by ToJSValue to check which scriptable interface is implemented. */
  using ScriptableInterfaceType = nsIDirectoryService;

  /* void init (); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD Init(void) = 0;

  /* void registerProvider (in nsIDirectoryServiceProvider prov); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD RegisterProvider(nsIDirectoryServiceProvider *prov) = 0;

  /* void unregisterProvider (in nsIDirectoryServiceProvider prov); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD UnregisterProvider(nsIDirectoryServiceProvider *prov) = 0;
