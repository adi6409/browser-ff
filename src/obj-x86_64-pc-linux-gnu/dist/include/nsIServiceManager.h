...
class NS_NO_VTABLE nsIServiceManager : public nsISupports {
 public:
  ...
  using ScriptableInterfaceType = nsIServiceManager;

  /* void getService (in nsCIDRef aClass, in nsIIDRef aIID, [iid_is (aIID), retval] out nsQIResult result); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD GetService(const nsCID & aClass, const nsIID & aIID, void * * result) = 0;

  /* void getServiceByContractID (in string aContractID, in nsIIDRef aIID, [iid_is (aIID), retval] out nsQIResult result); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD GetServiceByContractID(const char * aContractID, const nsIID & aIID, void * * result) = 0;

  /* boolean isServiceInstantiated (in nsCIDRef aClass, in nsIIDRef aIID); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD IsServiceInstantiated(const nsCID & aClass, const nsIID & aIID, bool *_retval) = 0;

  /* boolean isServiceInstantiatedByContractID (in string aContractID, in nsIIDRef aIID); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD IsServiceInstantiatedByContractID(const char * aContractID, const nsIID & aIID, bool *_retval) = 0;
