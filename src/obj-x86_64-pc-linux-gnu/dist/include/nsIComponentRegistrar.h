...
class NS_NO_VTABLE nsIComponentRegistrar : public nsISupports {
 public:
  ...
  using ScriptableInterfaceType = nsIComponentRegistrar;

  /* void autoRegister (in nsIFile aSpec); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD AutoRegister(nsIFile *aSpec) = 0;

  /* void autoUnregister (in nsIFile aSpec); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD AutoUnregister(nsIFile *aSpec) = 0;

  /* void registerFactory (in nsCIDRef aClass, in string aClassName, in string aContractID, in nsIFactory aFactory); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD RegisterFactory(const nsCID & aClass, const char * aClassName, const char * aContractID, nsIFactory *aFactory) = 0;

  /* void unregisterFactory (in nsCIDRef aClass, in nsIFactory aFactory); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD UnregisterFactory(const nsCID & aClass, nsIFactory *aFactory) = 0;
  ...
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD RegisterFactoryLocation(const nsCID & aClass, const char * aClassName, const char * aContractID, nsIFile *aFile, const char * aLoaderStr, const char * aType) = 0;

  /* void unregisterFactoryLocation (in nsCIDRef aClass, in nsIFile aFile); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD UnregisterFactoryLocation(const nsCID & aClass, nsIFile *aFile) = 0;

  /* boolean isCIDRegistered (in nsCIDRef aClass); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD IsCIDRegistered(const nsCID & aClass, bool *_retval) = 0;

  /* boolean isContractIDRegistered (in string aContractID); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD IsContractIDRegistered(const char * aContractID, bool *_retval) = 0;

  /* Array<ACString> getContractIDs (); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD GetContractIDs(nsTArray<nsCString >& _retval) = 0;

  /* string CIDToContractID (in nsCIDRef aClass); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD CIDToContractID(const nsCID & aClass, char * *_retval) = 0;

  /* nsCIDPtr contractIDToCID (in string aContractID); */
  JS_HAZ_CAN_RUN_SCRIPT NS_IMETHOD ContractIDToCID(const char * aContractID, nsCID * * _retval) = 0;
...
#define NS_DECL_NSICOMPONENTREGISTRAR \
  ...
  NS_IMETHOD GetContractIDs(nsTArray<nsCString >& _retval) override; \
...
#define NS_DECL_NON_VIRTUAL_NSICOMPONENTREGISTRAR \
  ...
  nsresult GetContractIDs(nsTArray<nsCString >& _retval); \
...
#define NS_FORWARD_NSICOMPONENTREGISTRAR(_to) \
  ...
  NS_IMETHOD GetContractIDs(nsTArray<nsCString >& _retval) override { return _to GetContractIDs(_retval); } \
...
#define NS_FORWARD_SAFE_NSICOMPONENTREGISTRAR(_to) \
  ...
  NS_IMETHOD GetContractIDs(nsTArray<nsCString >& _retval) override { return !_to ? NS_ERROR_NULL_POINTER : _to->GetContractIDs(_retval); } \
