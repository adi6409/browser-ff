...
#ifndef PMediaTransportParent_h
#define PMediaTransportParent_h
...
public:
    typedef std::function<void(const WebrtcGlobalLog&)> GetIceLogResolver;
    typedef std::function<void(const NotReallyMovableButLetsPretendItIsRTCStatsCollection&)> GetIceStatsResolver;
...
public:
    MOZ_IMPLICIT PMediaTransportParent();
...
public:
    ...
    [[nodiscard]] bool
    SendOnAlpnNegotiated(const string& alpn);

    [[nodiscard]] bool
    SendOnGatheringStateChange(const int& state);

    [[nodiscard]] bool
    SendOnConnectionStateChange(const int& state);
...
namespace ipc {
template<>
struct IPDLParamTraits <mozilla::dom::PMediaTransportParent*>
{
    typedef mozilla::dom::PMediaTransportParent* paramType;
    static void
    Write(
            IPC::Message* aMsg,
...
#endif // ifndef PMediaTransportParent_h
