...
#ifndef PWindowGlobalParent_h
#define PWindowGlobalParent_h
...
private:
    ...
    friend class mozilla::dom::PInProcessParent;
protected:
...
public:
    typedef MozPromise<bool, ResponseRejectReason, true> MakeFrameRemotePromise;
    typedef MozPromise<PaintFragment, ResponseRejectReason, true> DrawSnapshotPromise;
    typedef MozPromise<mozilla::Maybe<nsCString>, ResponseRejectReason, true> GetSecurityInfoPromise;
    typedef std::function<void(const nsresult&)> ShareResolver;
    typedef std::function<void(const uint32_t&)> GetContentBlockingEventsResolver;
    typedef std::function<void(const bool&)> CheckPermitUnloadResolver;
...
public:
    MOZ_IMPLICIT PWindowGlobalParent();
...
public:
    ...
    Send__delete__(PWindowGlobalParent* actor);

    [[nodiscard]] bool
    SendMakeFrameLocal(
            const MaybeDiscardedBrowsingContext& aFrameContext,
            const uint64_t& aSwitchId);
    ...
    [[nodiscard]] RefPtr<MakeFrameRemotePromise>
    SendMakeFrameRemote(
            const MaybeDiscardedBrowsingContext& aFrameContext,
            ManagedEndpoint<mozilla::dom::PBrowserBridgeChild>&& aEndpoint,
            const TabId& aTabId,
    ...
    [[nodiscard]] RefPtr<DrawSnapshotPromise>
    SendDrawSnapshot(
            const mozilla::Maybe<IntRect>& aRect,
            const float& aScale,
            const nscolor& aBackgroundColor,
    ...
    [[nodiscard]] RefPtr<GetSecurityInfoPromise>
    SendGetSecurityInfo();

    [[nodiscard]] bool
    SendDispatchSecurityPolicyViolation(const nsString& aViolationEventJSON);

    [[nodiscard]] bool
    SendSaveStorageAccessPermissionGranted();

    [[nodiscard]] bool
    SendAddBlockedFrameNodeByClassifier(const MaybeDiscardedBrowsingContext& aNode);

    [[nodiscard]] bool
    SendResetScalingZoom();

    [[nodiscard]] bool
    SendSetContainerFeaturePolicy(FeaturePolicy* aContainerFeaturePolicy);
...
namespace ipc {
template<>
struct IPDLParamTraits <mozilla::dom::PWindowGlobalParent*>
{
    typedef mozilla::dom::PWindowGlobalParent* paramType;
    static void
    Write(
            IPC::Message* aMsg,
...
#endif // ifndef PWindowGlobalParent_h
