...
#ifndef PBrowserBridgeChild_h
#define PBrowserBridgeChild_h
...
public:
    MOZ_IMPLICIT PBrowserBridgeChild();

    virtual ~PBrowserBridgeChild();
...
public:
    ...
    static bool
    Send__delete__(PBrowserBridgeChild* actor);

    bool
    SendLoadURL(nsDocShellLoadState* aLoadState);

    bool
    SendResumeLoad(const uint64_t& aPendingSwitchID);

    bool
    SendShow(const OwnerShowInfo& info);
    ...
    bool
    SendDispatchSynthesizedMouseEvent(const WidgetMouseEvent& event);
...
namespace ipc {
template<>
struct IPDLParamTraits <mozilla::dom::PBrowserBridgeChild*>
{
    typedef mozilla::dom::PBrowserBridgeChild* paramType;
    static void
    Write(
            IPC::Message* aMsg,
...
#endif // ifndef PBrowserBridgeChild_h
