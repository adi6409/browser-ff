...
#ifndef PGMPVideoDecoderChild_h
#define PGMPVideoDecoderChild_h
...
public:
    MOZ_IMPLICIT PGMPVideoDecoderChild();
...
public:
    ...
    static bool
    Send__delete__(PGMPVideoDecoderChild* actor);

    bool
    SendDecoded(const GMPVideoi420FrameData& aDecodedFrame);

    bool
    SendReceivedDecodedReferenceFrame(const uint64_t& aPictureId);

    bool
    SendReceivedDecodedFrame(const uint64_t& aPictureId);
    ...
    bool
    SendParentShmemForPool(Shmem&& aEncodedBuffer);
...
namespace ipc {
template<>
struct IPDLParamTraits <mozilla::gmp::PGMPVideoDecoderChild*>
{
    typedef mozilla::gmp::PGMPVideoDecoderChild* paramType;
    static void
    Write(
            IPC::Message* aMsg,
...
#endif // ifndef PGMPVideoDecoderChild_h
