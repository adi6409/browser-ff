...
#ifndef PWebrtcTCPSocketParent_h
#define PWebrtcTCPSocketParent_h
...
protected:
    ...
    RecvAsyncOpen(
            ...
            const bool& aUseTls,
            const mozilla::Maybe<WebrtcProxyConfig>& aProxyConfig) = 0;
    virtual mozilla::ipc::IPCResult
    RecvWrite(nsTArray<uint8_t>&& aWriteData) = 0;
    virtual mozilla::ipc::IPCResult
    RecvClose() = 0;
public:
...
public:
    MOZ_IMPLICIT PWebrtcTCPSocketParent();

    virtual ~PWebrtcTCPSocketParent();
...
public:
    ...
    [[nodiscard]] bool
    SendOnClose(const nsresult& aReason);

    [[nodiscard]] bool
    SendOnConnected(const nsCString& aProxyType);

    [[nodiscard]] bool
    SendOnRead(const nsTArray<uint8_t>& aReadData);

    [[nodiscard]] static bool
    Send__delete__(PWebrtcTCPSocketParent* actor);
...
namespace ipc {
template<>
struct IPDLParamTraits <mozilla::net::PWebrtcTCPSocketParent*>
{
    typedef mozilla::net::PWebrtcTCPSocketParent* paramType;
    static void
    Write(
            IPC::Message* aMsg,
...
#endif // ifndef PWebrtcTCPSocketParent_h
