...
#ifndef PTRRServiceParent_h
#define PTRRServiceParent_h
...
public:
    MOZ_IMPLICIT PTRRServiceParent();

    virtual ~PTRRServiceParent();
...
public:
    ...
    Send__delete__(PTRRServiceParent* actor);

    [[nodiscard]] bool
    SendUpdatePlatformDNSInformation(const nsTArray<nsCString>& aSuffixList);

    [[nodiscard]] bool
    SendUpdateParentalControlEnabled(const bool& aEnabled);

    [[nodiscard]] bool
    SendClearDNSCache(const bool& aTrrToo);

    [[nodiscard]] bool
    SendSetDetectedTrrURI(const nsCString& aURI);
...
namespace ipc {
template<>
struct IPDLParamTraits <mozilla::net::PTRRServiceParent*>
{
    typedef mozilla::net::PTRRServiceParent* paramType;
    static void
    Write(
            IPC::Message* aMsg,
...
#endif // ifndef PTRRServiceParent_h
