...
#ifndef PHttpConnectionMgrParent_h
#define PHttpConnectionMgrParent_h
...
public:
    MOZ_IMPLICIT PHttpConnectionMgrParent();

    virtual ~PHttpConnectionMgrParent();
...
public:
    ...
    Send__delete__(PHttpConnectionMgrParent* actor);

    [[nodiscard]] bool
    SendDoShiftReloadConnectionCleanupWithConnInfo(const HttpConnectionInfoCloneArgs& aArgs);

    [[nodiscard]] bool
    SendUpdateCurrentTopLevelOuterContentWindowId(const uint64_t& aWindowId);
    ...
    SendAddTransactionWithStickyConn(
            PHttpTransactionParent* aTrans,
            const int32_t& aPriority,
            PHttpTransactionParent* aTransWithStickyConn);
...
namespace ipc {
template<>
struct IPDLParamTraits <mozilla::net::PHttpConnectionMgrParent*>
{
    typedef mozilla::net::PHttpConnectionMgrParent* paramType;
    static void
    Write(
            IPC::Message* aMsg,
...
#endif // ifndef PHttpConnectionMgrParent_h
